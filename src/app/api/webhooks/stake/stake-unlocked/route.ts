...
const StakeUnlockedPayloadSchema = z.object({
  eventId: z.string(),
  eventType: z.literal('stake-unlocked'),
...
async function handleStakeUnlocked(data: z.infer<typeof StakeUnlockedPayloadSchema>['data']) {
  // Log the stake unlocked event
  console.log('[Webhook] Stake unlocked:', {
  ...
  // TODO: Update stake position in database
  // TODO: Create unlock request with release time
  // TODO: Trigger notification to owner

  return { processed: true }
}

// POST /api/webhooks/stake/unlocked
export async function POST(request: NextRequest) {
  try {
    ...
    const parseResult = parseWebhookEvent('stake-unlocked', payload)
    ...
    const validationResult = StakeUnlockedPayloadSchema.safeParse(payload)
    ...
    // Process event (async)
    handleStakeUnlocked(validationResult.data.data).catch(console.error)
